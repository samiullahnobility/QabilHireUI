...
export class ApiService {
  ...
  get<T>(path: string): Observable<T> {
    return this.track(path, "GET", () =>
      this.http.get<T>(`${API_BASE_URL}/${path}`, { withCredentials: true }),
    );
  }
  ...
  put<TResponse, TRequest>(
    ...
    return this.track(path, "PUT", () =>
      this.http.put<TResponse>(`${API_BASE_URL}/${path}`, body, {
        withCredentials: true,
      }),
    );
  }
  ...
  delete<T>(path: string): Observable<T> {
    return this.track(path, "DELETE", () =>
      this.http.delete<T>(`${API_BASE_URL}/${path}`, { withCredentials: true }),
    );
  }
  ...
  private messageFor(path: string, method: string): string {
    ...
    if (/resumes\/[^/]+\/extracted-data$/.test(path))
      return "Saving your resume corrections...";
    if (/resumes\/[^/]+\/active$/.test(path))
      return "Setting your active resume...";
    if (/resumes\/[^/]+\/archive$/.test(path))
      return "Updating your resume archive...";
    if (path === "resumes" && method === "POST")
      return "Uploading your resume...";
    if (path.startsWith("resumes") && method === "DELETE")
      return "Deleting your resume...";
    if (path.startsWith("resumes") && method === "GET")
      return path === "resumes"
    ...
    if (path === "job-matches" && method === "POST")
      return "Analyzing your job match with AI...";
    if (path.startsWith("job-matches") && method === "GET")
      return "Loading your job match...";
