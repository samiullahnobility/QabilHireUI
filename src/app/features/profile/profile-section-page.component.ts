...
export class ProfileSectionPageComponent {
  ...
  readonly filteredSkillGroups=()=>{const q=this.skillSearch().trim().toLowerCase();return q?this.skillGroups.map(group=>({label:group.label,skills:group.skills.filter(skill=>skill.toLowerCase().includes(q))})).filter(group=>group.skills.length>0):this.skillGroups.map(group=>({label:group.label,skills:[...group.skills]}));};
  readonly skillCount=()=>this.filteredSkillGroups().reduce((total,group)=>total+group.skills.length,0);
  ...
  toggleSkill(skill:string):void{this.selectedSkills.has(skill)?this.selectedSkills.delete(skill):this.selectedSkills.add(skill);this.syncSkills();}
  isGroupExpanded(label:string):boolean{return this.skillSearch().trim().length>0||this.expandedGroups().has(label);}
  ...
  save():void{
    ...
    this.draft.save().pipe(finalize(()=>this.saving.set(false))).subscribe({next:()=>{this.form.markAsPristine();this.submitAttempted=false;this.notifications.success(`${this.title} updated.`);},error:error=>this.notifications.error(error,'Unable to update your profile.')});
  }
  private syncSkills():void{this.form.controls.skills.setValue([...this.selectedSkills].join(', '),{emitEvent:false});this.form.controls.skills.markAsDirty();}
  private populate():void{const p=this.draft.value();this.form.patchValue({...p,skills:p.skills.join(', '),skillLevel:p.skillLevel||''});this.selectedSkills.clear();p.skills.forEach(x=>this.selectedSkills.add(x));this.selected.clear();p.preferences.forEach(x=>this.selected.add(x));this.form.controls.skills.setValue([...this.selectedSkills].join(', '),{emitEvent:false});this.form.markAsPristine();}
}
