...
export class InterviewSetupPageComponent {
  ...
  readonly categories = ['HR', 'Technical', 'Behavioral', 'Project-based', 'Mixed'];
  ...
  constructor() {
    this.profile.load().subscribe({
      ...
      error: error => this.notifications.error(error, 'Unable to load your target role.')
    });
  }

  choose(control: 'category'|'difficulty'|'mode'|'responseMode', value: string): void {
  ...
  submit(): void {
    ...
    this.api.create(this.form.getRawValue()).pipe(finalize(() => this.submitting.set(false))).subscribe({
      next: session => void this.router.navigate(['/app/interviews/session', session.id]),
      error: error => this.notifications.error(error, 'Unable to generate your interview.')
    });
  }
}
