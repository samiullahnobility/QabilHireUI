...
export class ResumeEditPageComponent {
  ...
  readonly issues = computed(() => {
    ...
    if (!data.experience.length)
      issues.push("No experience entries were detected.");
    if (!data.education.length)
      issues.push("No education entries were detected.");
    if (!data.skills.length) issues.push("No skills were detected.");
    return issues;
  });
  readonly hasBlockingIssues = computed(() =>
    this.issues().some((issue) => issue.includes("valid email")),
  );
      ...
      .subscribe({
        next: (current) => {
          ...
          try {
            this.data.set(
              this.normalizeData(
                current.extractedJson
                  ? JSON.parse(current.extractedJson)
                  : this.emptyData(),
              ),
            );
          } catch {
            this.data.set(this.emptyData());
          }
        },
        error: (error) =>
          this.notifications.error(
            error,
            "Unable to load extracted resume data.",
          ),
      });
  }
  ...
  save(): void {
    ...
    this.api
      .updateExtractedData(current.id, JSON.stringify(this.data()))
      .pipe(
      ...
      .subscribe({
        ...
        error: (error) =>
          this.notifications.error(error, "Unable to save corrections."),
      });
  }
    ...
    this.api
      ...
      .subscribe({
        next: (resume) => {
          ...
          this.data.set(
            this.normalizeData(
              resume.extractedJson
                ? JSON.parse(resume.extractedJson)
                : this.emptyData(),
            ),
          );
        ...
        error: (error) =>
          this.notifications.error(
            error,
            "Unable to extract the resume again.",
          ),
      });
  }
  ...
  updateItem(field: EntryField, index: number, value: string): void {
    this.data.update((data) => ({
      ...data,
      [field]: data[field].map((item, itemIndex) =>
        itemIndex === index ? value : item,
      ),
    }));
  }

  addItem(field: EntryField): void {
    this.data.update((data) => ({ ...data, [field]: [...data[field], ""] }));
  }
  removeItem(field: EntryField, index: number): void {
    this.data.update((data) => ({
      ...data,
  ...
  updateCustomSection(
    ...
    this.data.update((data) => ({
      ...data,
      customSections: data.customSections.map((section, sectionIndex) =>
        sectionIndex === index
          ? {
              ...section,
              [field]:
  ...
  private normalizeData(value: any): ResumeExtractedData {
    if (!Array.isArray(value?.sections))
      return {
        ...this.emptyData(),
        ...value,
        contact: { ...this.emptyData().contact, ...value?.contact },
    ...
    for (const section of value.sections) {
      ...
      if (category === "summary")
        data.summary = section.items?.join("\n") ?? "";
      else if (category in data)
        (data[category] as string[]) = section.items ?? [];
    }
