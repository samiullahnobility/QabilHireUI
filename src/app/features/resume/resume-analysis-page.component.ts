...
export class ResumeAnalysisPageComponent {
  ...
  readonly activeTab = signal<
    "overview" | "extracted" | "strengths" | "missing" | "keywords"
  >("overview");
    ...
    this.api
      ...
      .subscribe({
        next: (resume) => {
          ...
          try {
            this.analysis.set(
              resume.analysisJson ? JSON.parse(resume.analysisJson) : null,
            );
          } catch {
        ...
        error: (error) =>
          this.notifications.error(error, "Unable to load resume analysis."),
      });
  }
}
