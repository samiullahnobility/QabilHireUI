...
@Component({standalone:true,selector:'app-profile-career-goals-page',imports:[ReactiveFormsModule,MatFormFieldModule,MatInputModule,MatButtonModule],templateUrl:'./profile-career-goals-page.component.html',styleUrl:'./profile-setup.shared.css',changeDetection:ChangeDetectionStrategy.OnPush})
export class ProfileCareerGoalsPageComponent{
 ...
 submit():void{this.submitAttempted=true;if(this.form.invalid){this.form.markAllAsTouched();return;}this.draft.update({...this.form.getRawValue(),preferences:[...this.selected]});this.form.markAsPristine();this.submitAttempted=false;void this.router.navigateByUrl('/onboarding/profile/review');}
}
