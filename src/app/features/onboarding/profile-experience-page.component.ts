...
@Component({standalone:true,selector:'app-profile-experience-page',imports:[ReactiveFormsModule,MatFormFieldModule,MatInputModule,MatButtonModule],templateUrl:'./profile-experience-page.component.html',styleUrl:'./profile-setup.shared.css',changeDetection:ChangeDetectionStrategy.OnPush})
export class ProfileExperiencePageComponent{
 ...
 submit():void{this.submitAttempted=true;if(this.form.invalid){this.form.markAllAsTouched();return;}const value=this.form.getRawValue();this.draft.update({...value,responsibilities:value.achievement});this.form.markAsPristine();this.submitAttempted=false;void this.router.navigateByUrl('/onboarding/profile/education');}
}
