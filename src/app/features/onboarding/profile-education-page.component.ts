...
@Component({
  standalone: true,
  selector: "app-profile-education-page",
...
export class ProfileEducationPageComponent {
  ...
  filteredGroups(): { label: string; skills: string[] }[] {
  ...
  isGroupExpanded(label: string): boolean {
    return this.search().trim().length > 0 || this.expandedGroups().has(label);
  }
  ...
  toggle(skill: string): void {
    this.selected.has(skill)
      ? this.selected.delete(skill)
      : this.selected.add(skill);
    this.dirty.set(true);
  }
  ...
  private persist(): void {
    this.draft.update({ skills: [...this.selected], skillLevel: this.level() });
